...
import {
  ...
  DocumentTextIcon, UsersIcon, AcademicCapIcon, BookOpenIcon,
  CalendarDaysIcon, Cog6ToothIcon, FolderOpenIcon,
  ArrowLeftOnRectangleIcon, ChartBarIcon, ArchiveBoxIcon,
...
const menuGroups = [
  {
    items: [
      ...
      { name: 'Riwayat Absensi', to: '/attendance/history', icon: ClockIcon },
    ],
  },
  {
    label: 'Master Data',
    items: [
      { name: 'Data Siswa', to: '/master/students', icon: UsersIcon },
      { name: 'Data Guru', to: '/master/teachers', icon: AcademicCapIcon },
      { name: 'Data Kelas', to: '/master/classes', icon: FolderOpenIcon },
      { name: 'Mata Pelajaran', to: '/master/subjects', icon: BookOpenIcon },
      { name: 'Jadwal', to: '/master/schedules', icon: CalendarDaysIcon },
      { name: 'Tahun Ajaran', to: '/master/school-years', icon: ArchiveBoxIcon },
    ],
  },
  {
    label: 'Manajemen',
    items: [
      { name: 'Kelola QR', to: '/qr/manage', icon: QrCodeIcon },
      { name: 'Laporan', to: '/reports', icon: ChartBarIcon },
    ],
  },
  {
    label: 'Izin',
...
<template>
  <div class="flex h-screen bg-slate-50">
    <Transition name="slide">
      <aside v-if="sidebarOpen" class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 lg:hidden overflow-y-auto">
        <nav class="flex flex-col h-full">
          <div class="flex items-center justify-between h-16 px-5 border-b border-white/5 shrink-0">
    ...
    <aside class="hidden lg:flex flex-col w-64 bg-slate-900 shrink-0">
      ...
      <nav class="flex-1 px-3 py-4 overflow-y-auto">
        <div class="pb-4 mb-5 border-b border-white/5">
          <p class="font-display text-3xl text-white tracking-tight leading-none tabular-nums" id="admin-clock">--:--</p>
          <p class="text-slate-400 text-xs mt-1.5" id="admin-date">---</p>
